...
import org.apache.spark.secco.execution.plan.computation.PushBasedCodegenExec
...
//            case (block, 0) => None
      ...
      case (block, idx) =>
        Seq(
          TableIterator(block, childrenSchemas(idx).toArray, isSorted = false)
        )
...
//  }
  ...
  test("FilterExec") {
    for (intVal <- Seq(1, 2)) {
      ...
      val pushBasedCodegenExec = computation.PushBasedCodegenExec(filterExec)(0)
      resultIterList =
  ...
  test("ProjectExec") {
    ...
    val pushBasedCodegenExec = computation.PushBasedCodegenExec(projectExec)(0)
    resultIterList = resultIterList :+ pushBasedCodegenExec.executeWithCodeGen()
  }
