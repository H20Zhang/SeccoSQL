...
class MapAndSetBlockTest extends FunSuite with BeforeAndAfter {
  ...
  val theArray: Array[Any] =
    Array("abc", 1, 3, 4.0, true, 3.3, "variableLength")
  ...
  val genericInternalRow2: GenericInternalRow = new GenericInternalRow(
    theArray2
  )
  val genericInternalRow3: GenericInternalRow = new GenericInternalRow(
    theArray3
  )
...
//    testMapBlock = mapBlockBuilder.build()
  ...
  test("test merge") {
    ...
    val mapSetMergedBlock =
      testMapBlock.merge(testSetBlock, maintainSortOrder = false)
    val setMapMergedBlock =
      testSetBlock.merge(testMapBlock, maintainSortOrder = false)
