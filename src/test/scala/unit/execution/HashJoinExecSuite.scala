...
import org.apache.spark.secco.execution.{BlockInputExec, BuildHashMapExec, HashJoinExec, PushBasedCodegenExec}
import org.apache.spark.secco.execution.storage.block.InternalBlock
import org.apache.spark.secco.execution.storage.row.InternalRow
import org.apache.spark.secco.expression.{Alias, And, Attribute, AttributeReference, EqualTo, Expression}
...
class HashJoinExecSuite extends FunSuite with BeforeAndAfter{
  ...
  before {
    ...
    val rightInputExec0 = BlockInputExec(childrenSchemas_right.head, rightChild0)
    val rightBuildExec0: BuildHashMapExec =
      BuildHashMapExec(rightInputExec0,
        condition0.children.map(exprs => exprs.children(1).asInstanceOf[Attribute]))
    ...
    val rightBuildExec1: BuildHashMapExec =
      BuildHashMapExec(rightInputExec1,
        condition1.children.map(exprs => exprs.children(1).asInstanceOf[Attribute]))
    ...
    val rightBuildExec2: BuildHashMapExec =
      BuildHashMapExec(rightInputExec2,
        condition2.children.map(exprs => exprs.children(1).asInstanceOf[Attribute]))
  ...
  test("test_HashJoinExec"){
    ...
    for(childIdx <- childIndices) {
      ...
      hashJoinExec = HashJoinExec(leftInputExec, rightBuildExecs(childIdx), keysArray(childIdx), None)
