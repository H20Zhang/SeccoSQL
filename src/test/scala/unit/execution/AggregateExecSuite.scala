...
import org.apache.spark.secco.expression.{Attribute, AttributeReference, Expression, NamedExpression}
...
//    val groupingExpressions0 = Array[NamedExpression](childrenSchemas.head.head)
    ...
    try {
      for (childIdx <- childIndices) {
        val inputExec = BlockInputExec(childrenSchemas(childIdx), blocks(childIdx))
        println("in test, before HashJoinExecExec")
        val aggregateExec = AggregateExec(groupingFunctionsSeq(childIdx), aggregateFunctionsSeq(childIdx), inputExec)
