...
class UnionIteratorSuite extends FunSuite with BeforeAndAfter{
  ...
  var unionIter: UnionIterator = _
  def showNextAndResults(iter: UnionIterator): Unit = {
    while(iter.hasNext){
      println("unionIter.next(): " + iter.next().copy())
    }
    println("unionIter.results(): " + iter.results())
    println()
  }
  ...
  test("basic_functions_not_sorted"){
    ...
    unionIter = UnionIterator(leftTableIter, rightTableIter)
    showNextAndResults(unionIter)
