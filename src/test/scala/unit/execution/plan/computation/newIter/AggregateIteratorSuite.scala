...
class AggregateIteratorSuite extends FunSuite with BeforeAndAfter{
  ...
  var aggregateIter: AggregateIterator = _
  ...
  private def getTableIterator(childIdx: Int, isSorted: Boolean = false) = {
    ...
    val tableIter = TableIterator(childBlock, childSchema, isSorted = isSorted)
    tableIter
  }
  
  test("basic_functions_simple"){
    val childIndices = ArrayBuffer[Int]()
    var groupingFunctionsSeq: Seq[Array[NamedExpression]] = Seq[Array[NamedExpression]]()
    var aggregateFunctionsSeq: Seq[Array[AggregateFunction]] = Seq[Array[AggregateFunction]]()
    ...
    val aggregateFunctions0 = Array[AggregateFunction](Max(childrenSchemas.head(4)))
    ...
    val aggregateFunctions1 = Array[AggregateFunction](Min(childrenSchemas(1)(1)))
    ...
    val aggregateFunctions2 = Array[AggregateFunction](Count(childrenSchemas(2)(1)))
    ...
    val aggregateFunctions3 = Array[AggregateFunction](Average(childrenSchemas(3)(1)))
    ...
    val aggregateFunctions4 = Array[AggregateFunction](Average(childrenSchemas(4)(1)))
    ...
    val aggregateFunctions5 = Array[AggregateFunction](Average(childrenSchemas(5)(1)))
    ...
    val aggregateFunctions6 = Array[AggregateFunction](Average(childrenSchemas(6).head))
    aggregateFunctionsSeq = aggregateFunctionsSeq :+ aggregateFunctions6

    for (childIdx <- childIndices){
      val tableIter = getTableIterator(childIdx)
      aggregateIter = AggregateIterator(tableIter, groupingFunctionsSeq(childIdx), aggregateFunctionsSeq(childIdx))
      assert(aggregateIter.isBreakPoint())
      showNextAndResults(aggregateIter)
      assert(!aggregateIter.hasNext)
    }
  }

  test("basic_functions_multiple_grouping_and_aggregation"){

//    for (childIdx <- blocks.indices){
//      val tableIter: TableIterator = getTableIterator(childIdx)
//      aggregateIter = AggregateIterator(tableIter, groupingFunctionsSeq(childIdx), aggregateFunctionsSeq(childIdx))
//      assert(aggregateIter.isBreakPoint())
//      assertThrows[NoSuchMethodException](aggregateIter.hasNext)
//      println(s"$childIdx results:(): " + aggregateIter.results())
//    }
