...
class UnsafeAndGenericBlockTest extends FunSuite with BeforeAndAfter {
  ...
  val theArray: Array[Any] = Array("abc", 1, 3, 4.0, true, null, "variableLength")
  ...
  test("test some basic functions") {
    ...
    // size
    assert(testUnsafeBlock.size() == 5 && testUnsafeBlock.size() == testGenericBlock.size())
  ...
  test("test sortBy") {
    ...
    val sortedRow0 = sortedUnsafeBlock.asInstanceOf[UnsafeInternalBlock].getRow(0)
    val sortedRow1 = sortedUnsafeBlock.asInstanceOf[UnsafeInternalBlock].getRow(1)
    val sortedRow2 = sortedUnsafeBlock.asInstanceOf[UnsafeInternalBlock].getRow(2)
    val sortedRow3 = sortedUnsafeBlock.asInstanceOf[UnsafeInternalBlock].getRow(3)
    val sortedRow4 = sortedUnsafeBlock.asInstanceOf[UnsafeInternalBlock].getRow(4)
  ...
  test("test merge") {
    ...
    val sotedExpectedString = Array[String]("abc", "abc", "abc", "abc", "bcd", "bcd", "bcd", "bcd", "ecd", "ecd")
    ...
    for (index <- mergedSortedBlockArray1.indices) {
      ...
      assert(row0.getString(0) == sotedExpectedString(index) && row0.getInt(1) == sotedExpectedInt(index))
      assert(row1.getString(0) == sotedExpectedString(index) && row1.getInt(1) == sotedExpectedInt(index))
    }
    ...
    val unsotedExpetedString = Array[String]("abc", "abc", "bcd", "bcd", "ecd", "abc", "abc", "bcd", "bcd", "ecd")
    ...
    for (index <- mergedUnSortedBlockArray0.indices) {
      ...
      assert(row0.getString(0) == unsotedExpetedString(index) && row0.getInt(1) == unsotedExpetedInt(index))
      assert(row1.getString(0) == unsotedExpetedString(index) && row1.getInt(1) == unsotedExpetedInt(index))
    }
  ...
  test("test performance of sortBy") {
    ...
    unsafeInternalRow.setNullAt(5)
    unsafeInternalRow.setString(6, "variableLength")
    val rows = Array[InternalRow](genericInternalRow, genericInternalRow2, genericInternalRow3, genericInternalRow4, unsafeInternalRow)
    ...
    info(s"The sort of $unsafeRowsNum rows UnsafeInternalBlock cost $costTime ms")
    ...
    info(s"The sort of $genericRowsNum rows GenericInternalBlock cost $costTime ms")
  }
