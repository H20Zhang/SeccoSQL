...
import org.apache.spark.dolphin.codegen.CodeAndComment
...
object CodeFormatter {
  ...
  def format(code: CodeAndComment, maxLines: Int = -1): String = {
    ...
    if (needToTruncate) {
      formatter.addLine(
        s"[truncated to $maxLines lines (total lines is ${lines.length})]"
      )
    }
    formatter.result()
  }
  ...
  def stripExtraNewLines(input: String): String = {
    ...
    input.split('\n').foreach { l =>
      val line = l.trim()
      val skip = line == "" && (lastLine == "" || lastLine
        .endsWith("{") || lastLine.endsWith("*/"))
      if (!skip) {
        code.append(line)
        code.append("\n")
      }
  ...
  def stripExtraNewLinesAndComments(input: String): String = {
    extraNewLinesRegexp.replaceAllIn(
      commentRegexp.replaceAllIn(input, ""),
      "\n"
    )
  }
}
...
private class CodeFormatter {
  ...
  private def addLine(line: String): Unit = {
    ...
    val indentChange = line.count(c => "({".indexOf(c) >= 0) - line.count(c =>
      ")}".indexOf(c) >= 0
    )
