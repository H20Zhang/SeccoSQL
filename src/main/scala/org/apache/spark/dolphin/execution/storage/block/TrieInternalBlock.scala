...
import org.apache.spark.dolphin.types.{BooleanType, DoubleType, FloatType, IntegerType, LongType, StringType, StructField, StructType}
import org.apache.spark.dolphin.util.BSearch
...
class TrieInternalBlock(
  ...
  private def getInt(idx: Int) = Utils._UNSAFE.getInt(valuesAddress + idx * elemSize + 4)
  private def getFloat(idx: Int) = Utils._UNSAFE.getFloat(valuesAddress + idx * elemSize + 4)
  private def getLong(idx: Int) = Utils._UNSAFE.getLong(valuesAddress + idx * elemSize)
  private def getDouble(idx: Int) = Utils._UNSAFE.getDouble(valuesAddress + idx * elemSize)
  private def getParentIndex(idx: Int) = Utils._UNSAFE.getLong(valuesAddress + idx * elemSize + 8).toInt
  private def getFirstChildIndex(idx: Int) = (Utils._UNSAFE.getLong(valuesAddress + idx * elemSize + 8) >> 32).toInt
  //  getters(0) = getString
  ...
  private def setInt(idx: Int, value: Any): Unit =
    Utils._UNSAFE.putInt(valuesAddress + idx * elemSize, value.asInstanceOf[Int])
  private def setFloat(idx: Int, value: Any): Unit =
    Utils._UNSAFE.putFloat(valuesAddress + idx * elemSize + 4, value.asInstanceOf[Float])
  private def setLong(idx: Int, value: Any): Unit =
    Utils._UNSAFE.putLong(valuesAddress + idx * elemSize, value.asInstanceOf[Long])
  private def setDouble(idx: Int, value: Any): Unit =
    Utils._UNSAFE.putDouble(valuesAddress + idx * elemSize, value.asInstanceOf[Double])
  ...
  override def get[T: ClassTag](key: InternalRow): Array[T] = {
    ...
    for(idx <- start until end) arrayBuffer.append(getters(j)(idx).asInstanceOf[T])
    arrayBuffer.toArray
  }
  ...
  TrieInternalBlock = {
    ...
    def buildMapArray(): Array[Int] = {
      ...
      if (dictionaryOrder.isEmpty)
        ...
        for (j <- indexMapArray) indexMapArray(j) = columnNamesOriginal.indexOf(columnNamesInOrder(j))
      }
      indexMapArray
    }
    ...
    val initialFixedLengthZoneAddress = Utils._UNSAFE.allocateMemory(initialSize.toLong)
    val initialVariableLengthZoneAddress = Utils._UNSAFE.allocateMemory(initialSize.toLong)
    ...
    def finalProcess(): Unit = {
      ...
      val fixedLengthZoneAddress = Utils._UNSAFE.allocateMemory(fixedLengthZoneSize)
      ...
      val variableLengthZoneAddress = Utils._UNSAFE.allocateMemory(variableLengthZoneSize)
      block.alterVariableLengthZone(variableLengthZoneAddress, variableLengthZoneSize)
    }
    ...
    val comparator = new InternalRowComparator(rowsSchema, indexNewToOldMapArray)
...
class TrieInternalBlockBuilder(schema: StructType) extends InternalBlockBuilder {
  ...
  override def build(): InternalBlock = TrieInternalBlock(internalRowBuffer.toArray, schema)
}
