...
import org.apache.spark.dolphin.execution.storage.row.{InternalRow,GenericInternalRow,UnsafeInternalRow}
...
class HashSetInternalBlock extends InternalBlock with SetLike {
   ...
   */
  override def sortBy(DictionaryOrder: Seq[String]): InternalBlock = throw new Exception("sort is not allowed in HashSetInternalBlock")
  ...
  override def getDictionaryOrder: Option[Seq[String]] = throw new Exception("No dictionaryOrder in HashSetInternalBlock")
}

class HashSetInternalBlockBuilder(schema:StructType) extends InternalBlockBuilder {
  ...
  override def build(): HashSetInternalBlock = new HashSetInternalBlock(rows,schema)
}