package org.apache.spark.dolphin.execution.storage
...
object Utils {
  ...
  var sizeMap: Map[DataType, Long] = Map(IntegerType -> 4,
    LongType -> 8,
    BooleanType -> 1,
    FloatType -> 4,
    DoubleType -> 8,
  ...
  def calculateBitMapForRow(blockSchema: StructType, otherInternalRoW: InternalRow): Array[Long] = {
  ...
  def anyCompare(a: Any, b: Any): Int = {
    (a, b) match{
      case (aInt: Int, bInt: Int) => if(aInt < bInt) return -1 else if(aInt > bInt) return 1
      case (aBoolean: Boolean, bBoolean: Boolean) => if(aBoolean < bBoolean) return -1 else if(aBoolean > bBoolean) return 1
      case (aDouble: Double, bDouble: Double) => if(aDouble < bDouble) return -1 else if(aDouble > bDouble) return 1
      case (aFloat: Float, bFloat: Float) => if(aFloat < bFloat) return -1 else if(aFloat > bFloat) return 1
      case (aLong: Long, bLong: Long) => if(aLong < bLong) return -1 else if(aLong > bLong) return 1
      case (aString: String, bString: String) => if(aString < bString) return -1 else if(aString > bString) return 1
      case _ => throw new NotImplementedError(s"Data types not supported (${a.getClass} and ${b.getClass}")
    }
    return 0
  }
