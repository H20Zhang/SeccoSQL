...
import org.apache.spark.dolphin.execution.storage.row.{GenericInternalRow, InternalRow, UnsafeInternalRow}
...
class UnsafeInternalBlock extends InternalBlock with RowLike with UnsafeHelper {
  ...
  private var variableBaseOffset: Array[Long] = _ // store the its baseAddress if it's a variableLength area, else null
  ...
  def this(baseoffset: Long, variableBaseOffset: Array[Long], variableLengthRecord: Array[Long], blockSchema: StructType, size: Int) {
    this()
  ...
  private def getBoolean(address: Long, ordinal: Int): Boolean = _UNSAFE.getBoolean(null, address + ordinal * fieldByteSize + bitMapSize)

  private def getInt(address: Long, ordinal: Int): Int = _UNSAFE.getInt(null, address + ordinal * fieldByteSize + bitMapSize)

  private def getLong(address: Long, ordinal: Int): Long = _UNSAFE.getLong(null, address + ordinal * fieldByteSize + bitMapSize)

  private def getFloat(address: Long, ordinal: Int): Float = _UNSAFE.getFloat(null, address + ordinal * fieldByteSize + bitMapSize)

  private def getDouble(address: Long, ordinal: Int): Double = _UNSAFE.getDouble(null, address + ordinal * fieldByteSize + bitMapSize)
  ...
  private def getString(address: Long, ordinal: Int): String = {
    ...
    copyMemory(null, variableBaseOffset(ordinal) + offset, buf, BYTE_ARRAY_OFFSET, size)
    new String(buf)
  }

  private def getString(address: Long, theVariableBaseOffset: Array[Long], ordinal: Int): String = {
    val offsetAndSize: Long = getLong(address, ordinal)
    val offset: Long = offsetAndSize >>> 32
    val size: Int = offsetAndSize.toInt
    val buf: Array[Byte] = new Array[Byte](size)
    copyMemory(null, theVariableBaseOffset(ordinal) + offset, buf, BYTE_ARRAY_OFFSET, size)
    new String(buf)
  }
  ...
  private def getRowFromBaseOffset(theBaseOffset: Long, theVariableBaseOffset: Array[Long], i: Int): UnsafeInternalRow = {
    val unsafeInternalRow = new UnsafeInternalRow(blockSchema.fields.length)
    copyMemory(null, theBaseOffset + i * rowSize, null, unsafeInternalRow.baseOffset, rowSize) // copy the data of fixed-length area
    val variableFields = schema().fields.filter(_.dataType == StringType)
    for (index <- variableFields.indices) {
      val ordinal = blockSchema.indexOf(variableFields(index)) // the field index in
      if (!unsafeInternalRow.isNullAt(ordinal)) {
        val vStr = getString(theBaseOffset + i * rowSize, theVariableBaseOffset, ordinal)
        unsafeInternalRow.setNullAt(ordinal)
        unsafeInternalRow.setString(ordinal, vStr) // set the variable-length data
      }
    }
  ...
  override def sortBy(DictionaryOrder: Seq[String]): InternalBlock = {
    val priority = new Array[Int](DictionaryOrder.length) // store the index of the field that needs to be sorted
    ...
    for (index <- fields.indices) {
      ...
      if (dataType == StringType) {
        newVariableBaseOffset(index) = _UNSAFE.allocateMemory(variableLengthRecord(index))
        newVariableLengthRecord(index) = variableLengthRecord(index)
        copyMemory(null, variableBaseOffset(index), null, newVariableBaseOffset(index), variableLengthRecord(index))
      }
    }
    ...
    quickSort(newBaseOffset, priority, 0, blockSize) // sort in the new memory space
    val newUnsafeInternalBlock = new UnsafeInternalBlock(newBaseOffset, newVariableBaseOffset, newVariableLengthRecord, blockSchema, blockSize)
  ...
  private def largerThan(theBaseOffset: Long, priority: Array[Int], i: Int, otherInternalRow: InternalRow, theVariableBaseOffset: Array[Long], index: Int = 0): Boolean = {
    ...
    castType match {
      case DoubleType =>
        ...
        if (value_i > value_j) {
          ...
          return largerThan(theBaseOffset, priority, i, otherInternalRow, theVariableBaseOffset, index + 1)
        }
      case FloatType =>
      ...
      case StringType =>
        val value_i = getString(theBaseOffset + i * rowSize, theVariableBaseOffset, ordinal)
      ...
      case _ => throw new Exception("Comparison uses dataType that is not allowed!")
    }
    false
  }
  ...
  override def merge(other: InternalBlock,
    ...
    if (maintainSortOrder) {
      ...
      assert(other.getDictionaryOrder.nonEmpty, "the target block is not sorted!")
      assert(dictionaryOrder.equals(other.getDictionaryOrder), "the two blocks are not sorted in the same way!")

      // allocate new memory space for fixed-length data of the new block
      val newBaseOffset = _UNSAFE.allocateMemory((size() + other.size()) * rowSize)
      ...
      for (i <- 0 until other.size().toInt) {
        ...
        for (j <- 0 until variableNum) {
          ...
          val targetStringBA = otherInternalRow.getString(ordinal).map(_.toByte).toArray
          ...
          newVariableBaseOffset(ordinal) = _UNSAFE.reallocateMemory(newVariableBaseOffset(ordinal),
            newVariableLengthRecord(ordinal))
          setString(newVariableBaseOffset(ordinal) + offset, targetStringBA, stringSize)
      ...
      while (p1 >= 0 || p2 >= 0) {
        ...
        if (p1 == (-1)) {
          ...
          val unsafeInternalRow_p2 = this.getRow(p2).asInstanceOf[UnsafeInternalRow]
          // val otherUnsafeInternalRow_p1 = getRowFromBaseOffset(newBaseOffset, newVariableBaseOffset, p1)
          if (largerThan(newBaseOffset, priority, p1, unsafeInternalRow_p2, newVariableBaseOffset)) {
            copyDataFromUnsafeInternalRow(newBaseOffset, tail, newBaseOffset, p1)
            p1 -= 1
          }
          else {
      ...
      new UnsafeInternalBlock(newBaseOffset, newVariableBaseOffset, newVariableLengthRecord, blockSchema, newBlockSize)
    }
    else {
   ...
   */
  private def quickSort(theBaseOffset: Long, priority: Array[Int], left: Int, right: Int): Unit = {
    if (left < right) {
  ...
  private def partition(theBaseOffset: Long, priority: Array[Int], left: Int, right: Int): Int = {
    val pivot = left
    var j = pivot + 1 // recorde the index of next row that is larger than the row gotten by pivot
    var i = j
    ...
    firstDataType match {
      case DoubleType =>
        while (i < right) {
          ...
          val value_pivot = getDouble(theBaseOffset + pivot * rowSize, firstOrdinal)
      ...
      case FloatType =>
        while (i < right) {
          ...
          val value_pivot = getFloat(theBaseOffset + pivot * rowSize, firstOrdinal)
          if (value_i < value_pivot) {
            swap(theBaseOffset, i, j)
            j += 1
          }
      ...
      case IntegerType =>
        while (i < right) {
          ...
          val value_pivot = getInt(theBaseOffset + pivot * rowSize, firstOrdinal)
          if (value_i < value_pivot) {
            swap(theBaseOffset, i, j)
            j += 1
          }
      ...
      case LongType =>
        while (i < right) {
          ...
          val value_pivot = getLong(theBaseOffset + pivot * rowSize, firstOrdinal)
          if (value_i < value_pivot) {
            swap(theBaseOffset, i, j)
            j += 1
          }
      ...
      case StringType =>
        while (i < right) {
          ...
          val value_pivot = getString(theBaseOffset + pivot * rowSize, firstOrdinal)
          if (value_i < value_pivot) {
            swap(theBaseOffset, i, j)
            j += 1
          }
      ...
      case BooleanType =>
        while (i < right) {
          ...
          val value_pivot = getBoolean(theBaseOffset + pivot * rowSize, firstOrdinal)
          if (value_i < value_pivot) {
            swap(theBaseOffset, i, j)
            j += 1
          }
   ...
   */
  private def smallerThan(theBaseOffset: Long, priority: Array[Int], i: Int, j: Int, index: Int = 0): Boolean = {
    if (index == priority.length) return false // comparison ends
    val ordinal = priority(index)
  ...
  private def swap(theBaseOffset: Long, i: Int, j: Int): Unit = {
    ...
    copyMemory(null, theBaseOffset + i * rowSize, null, theBaseOffset + j * rowSize, rowSize)
   ...
   */
  private def copyDataFromInternalRow(theBaseOffset: Long, i: Int, otherInternalRow: InternalRow): Unit = {
   ...
   */
  private def copyDataFromUnsafeInternalRow(theBaseOffset: Long, i: Int, sourBaseOffset: Long, sourceIndex: Int): Unit = {
    val destinationBaseOffset = theBaseOffset + i * rowSize
    copyMemory(null, sourBaseOffset + sourceIndex * rowSize, null, destinationBaseOffset, rowSize)
  }
...
object UnsafeInternalBlock extends UnsafeHelper {
  ...
  def apply(rows: Array[InternalRow], schema: StructType): UnsafeInternalBlock = {
    val variableFields = schema.fields.filter(_.dataType == StringType)
    val variableNum = variableFields.length
    ...
    for (structField <- schema.fields) {
      ...
      if (structField.dataType == StringType) {
        variableBaseOffset(index) = _UNSAFE.allocateMemory(variableLengthRecord(index))
      }
      else {
    ...
    for (row <- rows) {
      ...
      for (index <- 0 until variableNum) {
        ...
        if (vStr != null) {
          ...
          setString(variableBaseOffset(indexInRow) + offsetRecord(indexInRow), dataByteArray, size) // set the data into variable-data are
    ...
    new UnsafeInternalBlock(blockBaseOffset, variableBaseOffset, variableLengthRecord, schema, rows.length)
  }

  /** Return the builder for building [[UnsafeInternalBlock]] */
  def builder(schema: StructType): UnsafeInternalRowBlockBuilder = new UnsafeInternalRowBlockBuilder(schema)
}

class UnsafeInternalRowBlockBuilder(schema: StructType) extends InternalBlockBuilder with UnsafeHelper {
...
trait UnsafeHelper {
  ...
  private[block] val BYTE_ARRAY_OFFSET = _UNSAFE.arrayBaseOffset(classOf[Array[Byte]])
  ...
  private[storage] def calculateBitMapWidthInWords(numFields: Int): Int = (numFields + 63) / 64

  private[storage] def copyMemory(src: AnyRef, srcOffset: Long, dst: AnyRef, dstOffset: Long, length: Long): Unit = {
    // Check if dstOffset is before or after srcOffset to determine if we should copy
    // forward or backwards. This is necessary in case src and dst overlap.
    _UNSAFE.copyMemory(src, srcOffset, dst, dstOffset, length)
  }

  def setString(variableBaseAddress: Long, value: Array[Byte], stringSize: Long): Unit = {
    copyMemory(value, BYTE_ARRAY_OFFSET, null, variableBaseAddress, stringSize)

  }

  def copyDataFromRowToBlock(schema: StructType, baseOffset: Long, i: Int, otherInternalRow: InternalRow): Unit = {
    ...
    for (i <- bitMapData.indices) _UNSAFE.putLong(null, destinationBaseOffset + i * 8 - bitMapSize, bitMapData(i))
    for (ordinal <- fields.indices) {
      val dataType = fields(ordinal).dataType
      dataType match {
        case DoubleType => _UNSAFE.putDouble(null, destinationBaseOffset + ordinal * fieldByteSize, otherInternalRow.getDouble(ordinal))
        case FloatType => _UNSAFE.putFloat(null, destinationBaseOffset + ordinal * fieldByteSize, otherInternalRow.getFloat(ordinal))
        case IntegerType => _UNSAFE.putInt(null, destinationBaseOffset + ordinal * fieldByteSize, otherInternalRow.getInt(ordinal))
        case LongType => _UNSAFE.putLong(null, destinationBaseOffset + ordinal * fieldByteSize, otherInternalRow.getLong(ordinal))
        case StringType => _UNSAFE.putLong(null, destinationBaseOffset + ordinal * fieldByteSize, otherInternalRow.getLong(ordinal))
        case BooleanType => _UNSAFE.putBooleanVolatile(null, destinationBaseOffset + ordinal * fieldByteSize, otherInternalRow.getBoolean(ordinal))
        case _ => throw new Exception("find dataType that is not allowed during swap!")
      }
    }
  }
}