...
import org.apache.spark.dolphin.execution.storage.row.{
  GenericInternalRow,
  InternalRow,
  UnsafeInternalRow
}
import org.apache.spark.dolphin.expression.{Attribute, Expression}
...
class CodegenContext {
  ...
  def addMutableState(
    ...
    if (forceInline || canInlinePrimitive || javaType.contains("[][]")) {
      ...
      val arrays = arrayCompactedMutableStates.getOrElseUpdate(
        javaType,
        new MutableStateArrays
      )
...
//      case _: StructType | _: ArrayType | _: MapType => code"$value = $initCode.copy();"
    ...
    ExprCode(code, FalseLiteralValue, JavaCode.global(value, dataType))
  }

  def declareMutableStates(): String = {
    // It's possible that we add same mutable state twice, e.g. the `mergeExpressions` in
    // `TypedAggregateExpression`, we should call `distinct` here to remove the duplicated ones.
    val inlinedStates = inlinedMutableStates.distinct.map {
      case (javaType, variableName) =>
        s"private $javaType $variableName;"
    }

    val arrayStates = arrayCompactedMutableStates.flatMap {
      case (javaType, mutableStateArrays) =>
        val numArrays = mutableStateArrays.arrayNames.size
        mutableStateArrays.arrayNames.zipWithIndex.map {
          case (arrayName, index) =>
  ...
  /**
    ...
    */
  val partitionInitializationStatements: mutable.ArrayBuffer[String] =
...
//  var subExprEliminationExprs = Map.empty[Expression, SubExprEliminationState]
  ...
  def addNewFunction(
      funcName: String,
      funcCode: String,
      inlineToOuterClass: Boolean = false
  ): String = {
    val newFunction =
      addNewFunctionInternal(funcName, funcCode, inlineToOuterClass)
    newFunction match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          s"$funcName is not matched at addNewFunction"
        )
    }
  }
...
//    case NullType => "false"
      case _ =>
        throw new IllegalArgumentException(
          "cannot generate equality code for un-comparable type: " + dataType
        )
    }
...
//    case udt: UserDefinedType[_] => genComp(udt.sqlType, c1, c2)
      case _ =>
        throw new IllegalArgumentException(
          "cannot generate compare code for un-comparable type: " + dataType
        )
    }
  ...
  /**
    ...
    */
  def reassignIfSmaller(
      dataType: DataType,
      partialResult: ExprCode,
      item: ExprCode
  ): String = {
    s"""
  ...
  def splitExpressions(
    ...
    if (blocks.length == 1) {
      ...
      val argString =
        arguments.map { case (t, name) => s"$t $name" }.mkString(", ")
  ...
  private def generateInnerClassesFunctionCalls(
    ...
    val innerClassToFunctions =
      mutable.LinkedHashMap.empty[(String, String), Seq[String]]
    functions.foreach(f => {
      val key = (f.innerClassName.get, f.innerClassInstance.get)
      val value = f.functionName +: innerClassToFunctions
    ...
    innerClassToFunctions.flatMap {
      case ((innerClassName, innerClassInstance), innerClassFunctions) =>
        ...
        if (orderedFunctions.size > MERGE_SPLIT_METHODS_THRESHOLD) {
          ...
          //     }
          //   }
          val body = foldFunctions(
            orderedFunctions.map(name => s"$name($argInvocationString)")
          )
          val code = s"""
...
/**
  ...
  * expressions.
  *
  * Process: bind -> cannoicalize -> create
  *
  * The function calling tree when generating the class
  *   a. generate(expressions, inputSchema)
  *   a. generate(bind(expressions, inputSchema))
  *   a. create(canonicalize(expression))
  */
...
//    val maxLines = SQLConf.get.loggingMaxLinesForCodegen
  ...
  /**
    ...
    */
  private def updateAndGetCompilationStats(
    ...
    val classes = {
      ...
      classesField
        .get(loader)
        .asInstanceOf[JavaMap[String, Array[Byte]]]
        .asScala
    }
...
//      case t: DecimalType => s"$input.getDecimal($ordinal, ${t.precision}, ${t.scale})"
      case StringType => s"$input.getString($ordinal)"
...
//      case t: DecimalType => s"$vector.putDecimal($rowId, $value, ${t.precision});"
      ...
      case _ =>
        throw new IllegalArgumentException(
          s"cannot generate code for unsupported type: $dataType"
        )
    }
  }
...
//    case DateType => java.lang.Integer.TYPE
//    case TimestampType => java.lang.Long.TYPE
