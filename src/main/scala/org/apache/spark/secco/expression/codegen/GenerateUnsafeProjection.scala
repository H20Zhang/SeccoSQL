...
import org.apache.spark.secco.execution.storage.row.{InternalRow, UnsafeInternalRow, UnsafeInternalRowWriter}
import org.apache.spark.secco.expression.BindReferences._
import org.apache.spark.secco.expression._
import org.apache.spark.secco.types._

/**
  * A projection that returns UnsafeInternalRow.
  *
  * CAUTION: the returned projection object should *not* be assumed to be thread-safe.
  */
abstract class UnsafeProjection extends Projection {
  override def apply(row: InternalRow): UnsafeInternalRow
}


/**
  * Generates a [[Projection]] that returns an [[UnsafeInternalRow]].
  *
  ...
  * form of UnsafeInternalRow (the scratch buffer will grow as needed).
  *
  * @note The returned UnsafeInternalRow will be pointed to a scratch buffer inside the projection.
  */
...
//    }
//
//    val rowWriterClass = classOf[UnsafeInternalRowWriter].getName
...
//       |if ($tmpInput instanceof UnsafeInternalRow) {
//       |  $rowWriter.write($index, (UnsafeInternalRow) $tmpInput);
//       |} else {
...
//
  private def writeExpressionsToBuffer(
    ...
    val resetWriter = if (isTopLevel) {
      ...
      if (inputs.map(_.isNull).forall(_ == FalseLiteralValue)) {
...
//        val dt = UserDefinedType.sqlType(dataType)
        ...
        // edited by lgh
        val setNull = s"$rowWriter.setNullAt($index);"
//          dt match {
...
//        }
        ...
        val writeField = writeElement(ctx, input.value, index.toString, dataType, rowWriter)
        if (!nullable) {
          s"""
             |${input.code}
             |${writeField.trim}
...
//      s"$writer.write($index, $input, $precision, $scale);"
  ...
  def createCode(
    ...
    val numVarLenFields = exprSchemas.count {
      case Schema(dt, _) => !UnsafeInternalRow.isFixedLength(dt)
    ...
    ExprCode(code, FalseLiteralValue, JavaCode.expression(s"$rowWriter.getRow()", classOf[UnsafeInternalRow]))
  }

  //TODO: currently we do nothing in method canonicalize
  ...
  private def create(
    ...
    val codeBody =
      s"""
         ...
         |  }
         |
         |  public UnsafeInternalRow apply(InternalRow ${ctx.INPUT_ROW}) {
         |    ${eval.code}
         |    return ${eval.value};
         |  }
         |
