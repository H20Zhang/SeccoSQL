...
abstract class BaseIteratorProducer {
...
/**
  ...
  */
object GenerateLeapFrogJoinIterator extends CodeGenerator[(Seq[Attribute], Seq[Seq[Attribute]]), BaseIteratorProducer]
{

  override protected def create(in: (Seq[Attribute], Seq[Seq[Attribute]])): BaseIteratorProducer = {
    create(in._1, in._2)
  }
  ...
  def create(schema: Seq[Attribute], childrenSchemas: Seq[Seq[Attribute]]): BaseIteratorProducer = {
    ...
    clazz.generate(Array.empty).asInstanceOf[BaseIteratorProducer]
  }
}
