...
import org.apache.spark.secco.errors.QueryCompilationErrors
...
//    Set("f", "false", "n", "no", "0").map(UTF8String.fromString)
  ...
  // edited by lgh: UTF8String -> String, trimAll -> trim
  def isTrueString(s: String): Boolean = trueStrings.contains(s.trim().toLowerCase)

  // edited by lgh: UTF8String -> String, trimAll -> trim
  def isFalseString(s: String): Boolean = falseStrings.contains(s.trim().toLowerCase)
