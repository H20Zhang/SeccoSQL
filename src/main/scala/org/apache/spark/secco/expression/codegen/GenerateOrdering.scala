...
import org.apache.spark.secco.codegen.{CodeAndComment, CodeGenerator, CodegenContext, ExprCode, ExpressionCanonicalizer}
...
object GenerateOrdering extends CodeGenerator[Seq[SortOrder], BaseOrdering] with Logging {
  ...
  //TODO: make the ExpressionCanonicalizer work, currently, we just bypass it as in GeneratePredicate --lgh
  protected def canonicalize(in: Seq[SortOrder]): Seq[SortOrder] = in
