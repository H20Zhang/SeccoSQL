...
import org.apache.spark.util.collection.unsafe.sort.PrefixComparators.{DoublePrefixComparator, StringPrefixComparator}
...
case class SortOrder(
  ...
  protected def withNewChildrenInternal(newChildren: IndexedSeq[Expression]): SortOrder =
...
case class SortPrefix(child: SortOrder) extends UnaryExpression {
  ...
  val nullValue = child.child.dataType match {
    case BooleanType | _: IntegralType =>
...
//        s"$DoublePrefixCmp.computePrefix($input.toDouble())"
  ...
  protected def withNewChildInternal(newChild: Expression): SortPrefix =
