...
abstract class UnsafeInternalRowJoiner {
  def join(row1: UnsafeInternalRow, row2: UnsafeInternalRow): UnsafeInternalRow
}


/**
  * A code generator for concatenating two [[UnsafeInternalRow]]s into a single [[UnsafeInternalRow]].
  *
  ...
  */
object GenerateUnsafeInternalRowJoiner extends CodeGenerator[(StructType, StructType), UnsafeInternalRowJoiner] {

  override protected def create(in: (StructType, StructType)): UnsafeInternalRowJoiner = {
    create(in._1, in._2)
  }
  ...
  def create(schema1: StructType, schema2: StructType): UnsafeInternalRowJoiner = {
    ...
    val updateOffset = (schema1 ++ schema2).zipWithIndex.map { case (field, i) =>
      ...
      if (UnsafeInternalRow.isFixedLength(field.dataType)) {
        ""
      } else {
        // Number of bytes to increase for the offset. Note that since in UnsafeInternalRow we store the
        ...
        // UnsafeInternalRow is a little underspecified, so in what follows we'll treat UnsafeInternalRowWriter's
        ...
        // UnsafeInternalRowWriter.setNullAt(ordinal) sets the null bit and stores 0 to in field's
        ...
        //      count bytes from the start of the UnsafeInternalRow so these offsets will always be
        ...
        //      UnsafeInternalRowWriter unconditionally calls setOffsetAndSize(ordinal, numBytes) with
    ...
    val codeBody = s"""
                      ...
                      |  return new SpecificUnsafeInternalRowJoiner();
                      |}
                      |
                      |class SpecificUnsafeInternalRowJoiner extends ${classOf[UnsafeInternalRowJoiner].getName} {
                      |  private byte[] buf = new byte[64];
                      |  private UnsafeInternalRow out = new UnsafeInternalRow(${schema1.size + schema2.size});
                      |
                      |  ${ctx.declareAddedFunctions()}
                      |
                      |  public UnsafeInternalRow join(UnsafeInternalRow row1, UnsafeInternalRow row2) {
                      ...
                      |    $updateOffsets
                      |
                      |    out.initWithByteArray(buf, sizeInBytes);
                      |
                      |    return out;
                      |  }
                      |}
    ...
    logDebug(s"SpecificUnsafeInternalRowJoiner($schema1, $schema2):\n${CodeFormatter.format(code)}")

    val (clazz, _) = CodeGenerator.compile(code)
    clazz.generate(Array.empty).asInstanceOf[UnsafeInternalRowJoiner]
  }
}
