...
//TODO: implement the codegen for this class.
...
/** An operator that perform aggregation
  ...
  * @param aggregateFunctions expressions to perform aggregation per group
  */
case class LocalAggregateExec(
    child: LocalProcessingExec,
    output: Seq[Attribute],
    groupingExpressions: Seq[NamedExpression],
    aggregateFunctions: Seq[AggregateFunction]
) extends LocalProcessingExec
  ...
  override def relationalSymbol: String =
    s"Agg[${groupingExpressions
      .map(_.sql)
      .mkString(",")}, ${aggregateFunctions.map(_.sql(false)).mkString(",")}]"
