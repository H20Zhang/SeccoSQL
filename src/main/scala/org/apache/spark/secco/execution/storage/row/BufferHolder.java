...
final class BufferHolder {
    ...
    private static final int ARRAY_MAX = Integer.MAX_VALUE - 15;

    // buffer is guarantee to be word-aligned since UnsafeRow assumes each field is word-aligned.
    private byte[] buffer;
    private int cursor = UnsafeInternalRow.BYTE_ARRAY_OFFSET();
    private final UnsafeInternalRow row;
    private final int fixedSize;

    BufferHolder(UnsafeInternalRow row) {
        this(row, 64);
    }

    BufferHolder(UnsafeInternalRow row, int initialSize) {
        int bitsetWidthInBytes = UnsafeInternalRow.calculateBitMapWidthInBytes(row.numFields());
        if (row.numFields() > (ARRAY_MAX - initialSize - bitsetWidthInBytes) / 8) {
            throw new UnsupportedOperationException(
                    "Cannot create BufferHolder for input UnsafeInternalRow because there are " +
                            "too many fields (number of fields: " + row.numFields() + ")");
        }
        this.fixedSize = bitsetWidthInBytes + 8 * row.numFields();
        int roundedSize = UnsafeInternalRow.roundNumberOfBytesToNearestWord(fixedSize + initialSize);
    ...
    void grow(int neededSize) {
        ...
        if (buffer.length < length) {
            ...
            int roundedSize = UnsafeInternalRow.roundNumberOfBytesToNearestWord(newLength);
    ...
    void reset() {
        cursor = UnsafeInternalRow.BYTE_ARRAY_OFFSET() + fixedSize;
    }

    int totalSize() {
        return cursor - UnsafeInternalRow.BYTE_ARRAY_OFFSET();
    }
}
