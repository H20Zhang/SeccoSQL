...
/**
 * A helper class to write data into global row buffer using `UnsafeInternalRow` format.
 *
 ...
 * changed, so we need to call `UnsafeInternalRowWriter.resetRowWriter` before writing, to update the
 ...
 */
public final class UnsafeInternalRowWriter extends UnsafeWriter {

    private final Unsafe _UNSAFE = UnsafeInternalRow.UNSAFE();

    private final UnsafeInternalRow row;

    private final int nullBitsSize;
    private final int fixedSize;

    public UnsafeInternalRowWriter(int numFields) {
        this(new UnsafeInternalRow(numFields));
    }

    public UnsafeInternalRowWriter(int numFields, int initialBufferSize) {
        this(new UnsafeInternalRow(numFields), initialBufferSize);
    }

    public UnsafeInternalRowWriter(UnsafeWriter writer, int numFields) {
        this(null, writer.getBufferHolder(), numFields);
    }

    private UnsafeInternalRowWriter(UnsafeInternalRow row) {
        this(row, new BufferHolder(row), row.numFields());
    }

    private UnsafeInternalRowWriter(UnsafeInternalRow row, int initialBufferSize) {
        this(row, new BufferHolder(row, initialBufferSize), row.numFields());
    }

    private UnsafeInternalRowWriter(UnsafeInternalRow row, BufferHolder holder, int numFields) {
        super(holder);
        this.row = row;
        this.nullBitsSize = UnsafeInternalRow.calculateBitMapWidthInBytes(numFields);
        this.fixedSize = nullBitsSize + 8 * numFields;
        this.startingOffset = cursor();
    }

    /**
     * Updates total size of the UnsafeInternalRow using the size collected by BufferHolder, and returns
     * the UnsafeInternalRow created at a constructor
     */
    public UnsafeInternalRow getRow() {
    ...
    public void zeroOutNullBytes() {
        for (int i = 0; i < nullBitsSize; i += 8) {
            _UNSAFE.putLong(getBuffer(), startingOffset + i, 0L);
        }
    }
...
//            _UNSAFE.putLong(getBuffer(), cursor(), 0L);
//            _UNSAFE.putLong(getBuffer(), cursor() + 8, 0L);
//
//            // Make sure Decimal object has the same scale as DecimalType.
...
//                _UNSAFE.copyMemory(
//                        bytes, UnsafeInternalRow.BYTE_ARRAY_OFFSET(), getBuffer(), cursor(), numBytes);
