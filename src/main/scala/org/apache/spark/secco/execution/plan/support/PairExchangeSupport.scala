...
trait PairExchangeSupport {
  self: SeccoPlan =>

  @transient protected var _shareResults: Option[ShareResults] = None

  def sharedShare: SharedParameter[mutable.HashMap[String, Int]]

  /** num of partition for each attribute */
  def share: mutable.HashMap[String, Int] = sharedShare.res

  /** vectors of hypercube */
  def shareVectors: Array[Array[Int]] = genShareVectorsForAttrs(outputOld)

  /** partitioner for self */
  override def taskPartitioner =
    new PairPartitioner(
      outputOld.map(share).toArray
    )
...
//  def partitionerForRelation(id: RelationID): PairPartitioner = {
//    val attrShareSpace =
//      query.idForRelation(id).schema.attrIDs.map(share).toArray
//    new PairPartitioner(attrShareSpace)
//  }
...
//
//    var maximalLoad = Conf.defaultConf().Secco_MEMORY_BUDGET
//
...
//
//      val load =
//        (communicationCost.toDouble / share.product) * 10 //10 Bytes per edge
//
