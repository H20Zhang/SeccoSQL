...
class UnsafeInternalRow extends InternalRow {
  ...
  private def assertIndexIsValid(index: Int): Unit = {
    ...
    assert (index < numFields, "index (" + index + ") should < " + numberOfFields)
  }
  ...
  private def setBitMapIfNotSet(index: Int): Unit = {
    ...
    val wordOffset = baseOffset + (index >> 6) * 8
    val word = _UNSAFE.getLong(baseObject, wordOffset)
    if((mask & word) == 0L)  _UNSAFE.putLong(baseObject, wordOffset, word | mask)
  }
  ...
  /**
    ...
    */
  private def copyMemory(src: AnyRef, srcOffset: Long, dst: AnyRef, dstOffset: Long, length: Long): Unit = {
  ...
  override def update(i: Int, value: Any): Unit = {
    ...
    if (!value.isInstanceOf[String]) {
      ...
      value match{
        case vBool: Boolean => _UNSAFE.putBoolean(baseObject, getFieldOffset(i), vBool)
        case vByte: Byte => _UNSAFE.putByte(baseObject, getFieldOffset(i), vByte)
        case vShort: Short => _UNSAFE.putShort(baseObject, getFieldOffset(i), vShort)
        case vInt: Int => _UNSAFE.putInt(baseObject, getFieldOffset(i), vInt)
        case vLong: Long => _UNSAFE.putLong(baseObject, getFieldOffset(i), vLong)
        case vFloat: Float => _UNSAFE.putFloat(baseObject, getFieldOffset(i), vFloat)
        case vDouble: Double => _UNSAFE.putDouble(baseObject, getFieldOffset(i), vDouble)
        case _ =>
      }    }
  ...
  override def setLong(i: Int, value: Long): Unit = _UNSAFE.putLongVolatile(baseObject, getFieldOffset(i), value)
  ...
  override def setString(i: Int, vStr: String): Unit = {
    ...
    if (!isNullAt(i)) {
      ...
      if (originalSize >= stringSize) {
        ...
        if (originalSize - stringSize > 7) {   // lgh: to avoid too many tiny fragments
          val fragOffsetAndSize = (originalOffset + stringSize) << 32 | originalSize - stringSize
    ...
    for (idx <- 0 to max_idx) {
      ...
      if (fragmentSize >= stringSize) {
        ...
        val updatedFragOffsetAndSize: Long = updatedFragOffset | fragmentSize.toLong
  ...
  override def getFloat(ordinal: Int): Float = _UNSAFE.getFloat(baseObject, getFieldOffset(ordinal))

  override def getDouble(ordinal: Int): Double = _UNSAFE.getDouble(baseObject, getFieldOffset(ordinal))
  ...
  override def toString: String = {
    ...
    while (i < sizeInBytes) {
      if (i != 0) build.append(',')
      build.append(java.lang.Long.toHexString(_UNSAFE.getLong(baseObject, baseOffset + i)))
  ...
  override def hashCode: Int = Murmur3_x86_32.hashUnsafeWords(baseObject, baseOffset, sizeInBytes, 42)
...
//    Utils._UNSAFE.copyMemory(baseRef, address, baseObject, getFieldOffset(i) + 8 - elemSize, elemSize)
  ...
  private[storage] def copyStringFrom(variableLengthZoneAddress: Long, offsetAndSize: Long,  i: Int): Unit =
  {
    ...
    Utils.copyMemory(null, stringAddress, baseObject, baseOffset + currentCursor, stringSize)
