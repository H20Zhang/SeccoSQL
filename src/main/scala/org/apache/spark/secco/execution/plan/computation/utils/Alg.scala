...
object Alg {
  ...
  def reorder(
    ...
    val tupleMappingFunc =
      attributeOrder
        ...
        .map(schema.indexOf(_))
        .toArray

    //TODO(optimize): ignore reordering if input is already ordered based on attributeOrder.
  ...
  def binarySearch[T](array: Array[T], value: T)(implicit
...
class LeapFrogUnaryIterator(var arrays: Array[ArraySegment])
  ...
  var maximalElement = Double.MinValue
  var isEnd = false
...
//    intersect the arrays
  ...
  def binaryMergeLikeIntersection(
      leftArray: ArraySegment,
