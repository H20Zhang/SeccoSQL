...
import org.apache.spark.secco.execution.MultiExecNode
import org.apache.spark.secco.execution.plan.computation.deprecated.BuildTrieExec
...
import org.apache.spark.secco.expression.{Attribute, BoundReference, Expression}
...
/** An operator that build Trie
  *
  * @param child child local operator
  */
case class BuildTrieExec(child: LocalProcessingExec, attrOrder: AttributeOrder)
  ...
  override protected def doProduceBulk(
    ...
    val codeStr = {
      s"""
         |$trieBuilderTerm = ($builderClassName) ${ctx.addReferenceObj(
        s"HashMapBuilder",
        new TrieInternalBlockBuilder(StructType.fromAttributes(attrOrder.order))
      )};
  ...
  override def iterator(): SeccoIterator =
    BuildTrie(child.iterator(), attrOrder.order)

  override def relationalSymbol: String = s"BuildTrie"
}
...
case class LocalLeapFrogJoinExec(
  ...
  override def iterator(): SeccoIterator = null
...
//  }
  ...
  protected override def doProduce(ctx: CodegenContext): String = {
    bindingTerm = ctx.addMutableState(
      "java.lang.Object[]",
      "binding",
      v => s"$v = new java.lang.Object[${numRepAttrs}];",
      forceInline = true
    )
    triesTerm = ctx.addMutableState(
      s"${classOf[TrieInternalBlock].getName}[]",
      "trieBlocks",
      v => s"$v = new ${classOf[TrieInternalBlock].getName}[${numChildren}];",
      forceInline = true
    )
    ...
    for (dt <- dataTypes) {
      val (className, classDefinition) =
        GenerateUnaryIterator.getUnaryIteratorCode(ctx, dt)
    ...
    for (i <- producerClassNames.indices) {
      val (className, classDefinition) = getUnaryIteratorProducerCode(
        ctx,
        i,
        children.map(_.asInstanceOf[PushBasedCodegen].output),
        iteratorClassNames(repAttrs(i).dataType)
      )
    ...
    val builtOnceTerm =
      ctx.addMutableState("boolean", "builtOnce", v => s"$v = false;")
    val loopedOnceTerm =
      ctx.addMutableState("boolean", "loopedOnce", v => s"$v = false;")
    ...
    s"""
      ...
      tupleSeq.map { case (codeStr, _) => codeStr }.mkString("\n")
       ...
       |      ${builtTrieTerms.zipWithIndex
      .map { case (term, idx) => s"$triesTerm[$idx] = $term;" }
      .mkString("\n")}
       |  }
       |  $builtOnceTerm = true;
       |}
  ...
  private def generateLeapFrogUnaryIteratorCode(
      ctx: CodegenContext,
      idx: Int
  ): String = {
    if (idx < numRepAttrs) {
      ...
      s"""
         |${curUnaryIterTerm} = (${iteratorClassNames(
        repAttrs(idx).dataType
      )})${producerTerms(idx)}.getIterator();
  ...
  """.stripMargin
      ...
      val naturalJoinLikeRowTerm =
        ctx.addMutableState(rowClassName, "outputRow")
      val outputVars = {
        // creating the vars will make the parent consume add an unsafe projection.
        ctx.INPUT_ROW = naturalJoinLikeRowTerm
        ctx.currentVars = null
        output.map { a =>
          val repAttrIdxForCurAttr =
            repAttrs.indexOf(attrOrder.equiAttrs.attr2RepAttr(a))
          BoundReference(repAttrIdxForCurAttr, a.dataType, a.nullable).genCode(
            ctx
          )
        }
      }
  ...
  private def getUnaryIteratorProducerCode(
      ctx: CodegenContext,
      curLevel: Int,
      childrenSchemas: Seq[Seq[Attribute]],
      unaryIteratorClassName: String
  ): (String, String) = {
    ...
    def getPrefixIndices(attrIdx: Int, childIdx: Int): Seq[Int] =
      relevantRelationIndicesForEachAttr
        .slice(0, attrIdx)
        .zipWithIndex
        .filter { case (childIndices, _) =>
          childIndices.contains(childIdx)
        }
        .map(_._2)
    ...
    val curRelevantRelationIndices = relevantRelationIndicesForEachAttr(
      curLevel
    )
    val numRelevantRelations = curRelevantRelationIndices.length
    val prefixIndicesForEachChild =
      curRelevantRelationIndices.map(getPrefixIndices(curLevel, _))

    val classDefinition =
      s"""
         |private class $className {
         |
         |    final DataType[] dataTypes = ${prefixSchema
        .map("DataTypes." + _.dataType)
        .mkString("{", ",", "}")};
         |    final int[] curRelevantRelationIndices = ${curRelevantRelationIndices
        .mkString("{", ",", "}")};
