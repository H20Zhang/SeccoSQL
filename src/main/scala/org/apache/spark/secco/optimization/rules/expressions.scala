...
import org.apache.spark.secco.types.IntegralType

/*
 * This file contains rules for optimizing expression in [[LogicalPlan]].
 *
 * 1. ConstantFolding: replace expression that can be statically evaluated with equivalent literal.
 * 2. ConstantPropagation: substitutes [[Attribute Attributes]] which can be statically evaluated with their
 * corresponding value in conjunctive [[Expression Expressions]].
 * 3. ReorderAssociativeOperator: reorder associative integral-type operators and fold all constants into one.
 * 4. (TBD) OptimizeIn: optimize IN predicates.
 * 5. BooleanSimplification: simplifies boolean expressions.
 * 6. SimplifyBinaryComparison: simplifies binary comparisons with semantically-equal expressions
 * 7. (TBD) SimplifyConditionals: simplifies conditional expressions (if / case).
 * 8. (TBD) LikeSimplification: simplifies LIKE expressions that do not need full regular expressions to evaluate the
 * condition.
 * 9. (TBD) NullPropagation
 * 10. (TBD) FoldablePropagation
 * 11. (TBD) SimplifyCasts
 * 12. RemoveDispensableExpressions
 * 13. (TBD) CombineConcats
 *
 */

/** A rule that replaces [[Expression]] that can be statically evaluated with
...
/** A rules that substitutes [[Attribute]] which can be statically evaluated with their corresponding
  * value in conjunctive [[Expression]]
  * eg.
  * {{{
...
object ConstantPropagation extends Rule[LogicalPlan] with PredicateHelper {
  def apply(plan: LogicalPlan): LogicalPlan = plan transform { case f: Filter =>
    val (newCondition, _) =
      traverse(f.condition, replaceChildren = true, nullIsFalse = true)
...
//        (None, Seq(((right, left), e)))
      ...
      case o: Or =>
        ...
        val (newRight, _) =
          traverse(o.right, replaceChildren = true, nullIsFalse)
        val newSelf = if (newLeft.isDefined || newRight.isDefined) {
          Some(
            Or(
              left = newLeft.getOrElse(o.left),
              right = newRight.getOrElse((o.right))
            )
          )
        } else {
          None
      ...
      case n: Not =>
        // Ignore the EqualityPredicates from children since they are only propagated through And.
        val (newChild, _) =
          traverse(n.child, replaceChildren = true, nullIsFalse = false)
...
object ReorderAssociativeOperator extends Rule[LogicalPlan] {
  private def flattenAdd(
      ...
      groupSet: ExpressionSet
  ): Seq[Expression] = expression match {
  ...
  private def collectGroupingExpressions(plan: LogicalPlan): ExpressionSet =
    plan match {
      case Aggregate(child, groupingExpressions, aggregateExpressions, mode) =>
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    case q: LogicalPlan =>
      ...
      q transformExpressionsDown {
        case a: Add
            if a.deterministic && a.dataType.isInstanceOf[IntegralType] =>
          val (foldables, others) =
            flattenAdd(a, groupingExpressionSet).partition(_.foldable)
          if (foldables.size > 1) {
            val foldableExpr = foldables.reduce((x, y) => Add(x, y))
            val c = Literal.create(foldableExpr.eval(EmptyRow), a.dataType)
            if (others.isEmpty) c
            else Add(others.reduce((x, y) => Add(x, y)), c)
          } else {
            a
          }
        case m: Multiply
...
//          } else if (
//            newList.length > SQLConf.get.optimizerInSetConversionThreshold
//          ) {
...
//        case a EqualNullSafe b if a.semanticEquals(b) => TrueLiteral
        ...
        case a GreaterThanOrEqual b
            if !a.nullable && !b.nullable && a.semanticEquals(b) =>
          TrueLiteral
        case a LessThanOrEqual b
            if !a.nullable && !b.nullable && a.semanticEquals(b) =>
          TrueLiteral
...
//          }
//
//        case CaseWhen(branches, _)
//            if branches.headOption.map(_._1).contains(TrueLiteral) =>
...
//          } else {
//            e.copy(branches =
//              branches.take(i).map(branch => (branch._1, elseValue))
//            )
//          }
//      }
//  }
...
//              GreaterThanOrEqual(
//                Length(input),
//                Literal(prefix.length + postfix.length)
//              ),
//              And(
//                StartsWith(input, Literal(prefix)),
//                EndsWith(input, Literal(postfix))
...
//          )
//        case e @ AggregateExpression(Count(exprs), _, _, _)
//            if exprs.forall(isNullLiteral) =>
//          Cast(
//            Literal(0L),
//            e.dataType,
//            Option(SQLConf.get.sessionLocalTimeZone)
//          )
//        case ae @ AggregateExpression(Count(exprs), _, false, _)
