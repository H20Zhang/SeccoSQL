...
import org.apache.spark.secco.expression.{Alias, Attribute, Expression}
import org.apache.spark.secco.expression.utils.AttributeMap
import org.apache.spark.secco.optimization.LogicalPlan
import org.apache.spark.secco.types.{BooleanType, DataType, DoubleType, FloatType, IntegerType, LongType, NumericType, StringType}
...
object EstimationUtils {
	...
	def rowCountsExist(plans: LogicalPlan*): Boolean =
		plans.forall(_.stats.rowCount != null)
	...
	def columnStatsExist(statsAndAttr: (Statistics, Attribute)*): Boolean = {
		statsAndAttr.forall { case (stats, attr) =>
			stats.attributeStats.contains(attr.name)
		}
	}
	...
	def columnStatsWithCountsExist(statsAndAttr: (Statistics, Attribute)*): Boolean = {
		statsAndAttr.forall { case (stats, attr) =>
			stats.attributeStats.get(attr.name).map(_.hasCountStats).getOrElse(false)
		}
	}
...
//										 attrStats: AttributeMap[ColumnStat] = AttributeMap(Nil)): BigInt = {
										 attrStats: mutable.HashMap[String, ColumnStat] = new mutable.HashMap[String, ColumnStat].empty): BigInt = {
		//		attrStats: ColumnStat): BigInt = {
		// We assign a generic overhead for a Row object, the actual overhead is different for different
		// Row format.
		8 + attributes.map { attr =>
			if (attrStats.get(attr.name).map(_.avgLen.isDefined).getOrElse(false)) {
				attr.dataType match {
					case StringType =>
						// UTF8String: base + offset + numBytes
						attrStats(attr.name).avgLen.get + 8 + 4
					case _ =>
						attrStats(attr.name).avgLen.get
				}
			} else {
...
//										 attrStats: AttributeMap[ColumnStat] = AttributeMap(Nil)): BigInt = {
										 attrStats: mutable.HashMap[String, ColumnStat] ): BigInt = {
...
//			case _: NumericType | DateType | TimestampType => value.toString.toDouble
			case _: NumericType => value.toString.toDouble
