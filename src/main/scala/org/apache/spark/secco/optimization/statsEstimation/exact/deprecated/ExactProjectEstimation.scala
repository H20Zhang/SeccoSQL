...
import org.apache.spark.secco.optimization.plan.{Join, Project, Relation}
...
object ExactProjectEstimation extends Estimation[Project] {
  ...
  override def estimate(x: Project): Option[Statistics] = {
    ...
    if (_selectivityMap.isEmpty) {
      ...
      val selectivityOpt =
        _selectivityMap.get.get((x.projectionListOld.toSet, relationNameSet))
