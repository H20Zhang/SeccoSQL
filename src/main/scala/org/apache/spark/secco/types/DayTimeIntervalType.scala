...
import org.apache.spark.secco.types.DayTimeIntervalType.fieldToString
...
case class DayTimeIntervalType(startField: Byte, endField: Byte) extends AnsiIntervalType {
	...
	override def asNullable: DayTimeIntervalType = this
...
//		.map(i => s"$i (${DayTimeIntervalType.fieldToString(i)})")
	...
	def fieldToString(field: Byte): String = field match {
		...
		case MINUTE => "minute"
		case SECOND => "second"
		case invalid => throw new AnalysisException(s"Invalid field id '$field' in day-time interval. " +
			s"Supported interval fields: ${dayTimeFields.mkString(", ")}.")
