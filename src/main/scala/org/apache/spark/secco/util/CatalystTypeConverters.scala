...
import org.apache.spark.secco.{GenericRowWithSchema, Row}
...
import org.apache.spark.secco.util._
...
object CatalystTypeConverters {
  ...
  private[secco] def isPrimitive(dataType: DataType): Boolean = {
    dataType match {
...
//  }
  ...
  private case class StructConverter(structType: StructType) extends CatalystTypeConverter[Any, Row, InternalRow] {
    ...
    override def toScalaImpl(row: InternalRow, column: Int): Row =
      throw QueryExecutionErrors.notSupportGetStructError()
//      toScala(row.getStruct(column, structType.size))
  }

  private object StringConverter extends CatalystTypeConverter[Any, String, String] {
    override def toCatalystImpl(scalaValue: Any): String = scalaValue match {
...
//      case ac: Array[Char] => UTF8String.fromString(String.valueOf(ac))
    ...
    override def toScala(catalystValue: String): String = catalystValue
